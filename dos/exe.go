...
// MS-DOS EXE header
// N.B. NOT memory layout compatible with the actual DOS structure
...
func ReadExeFromFile(filename string) (*Executable, error) {
	...
	if len(b) < 12 {
		return nil, errors.New("header too small for file: " + filename)
	}
...
func ReadExe(bs []byte) (*Executable, error) {
	...
	exe := &Executable{Etype: EXE, Exists: true}
	if bs[0] != 'M' || bs[1] != 'Z' {
		exe.Etype = COM
		exe.Data = bs
		return exe, nil
